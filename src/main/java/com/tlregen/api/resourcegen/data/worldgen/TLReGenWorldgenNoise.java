...
public class TLReGenWorldgenNoise extends TLReGenResourceGenerator {
	...
	public TLReGenWorldgenNoise(DynamicRegister<NoiseParameters> dynReg, String modID, PackOutput packOutput) {
		super(modID, Types.NOISE, packOutput);
		dynamicRegister = dynReg;
	}

	@Override
	public CompletableFuture<?> run(final CachedOutput cache) {
		return MasterResourceGenerator.lookupProvider.thenApply(r -> constructRegistries(r, new TLReGenRegistrySetBuilder().add(Registries.NOISE, TLReGenWorldgenNoise::bootstrap))).thenCompose((provider) -> {
