...
public class TLReGenWorldgenDensityFunction extends MasterResourceGenerator implements DataProvider {
	private final CompletableFuture<HolderLookup.Provider> completables = lookupProvider.thenApply(r -> constructRegistries(r, new TLReGenRegistrySetBuilder().add(Registries.DENSITY_FUNCTION, TLReGenWorldgenDensityFunction::bootstrap)));
	...
	public static void bootstrap(final BootstapContext<DensityFunction> bootstrapContext) {
		TLReGenWorldgenDensityFunction.setBootstrapContext(bootstrapContext);
	...
	protected static void setBootstrapContext(BootstapContext<DensityFunction> bootstrapContextIn) {
