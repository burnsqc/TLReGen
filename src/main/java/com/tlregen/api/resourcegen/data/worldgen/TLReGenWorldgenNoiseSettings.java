...
public abstract class TLReGenWorldgenNoiseSettings extends TLReGenMasterResourceGenerator implements DataProvider {
	private final CompletableFuture<HolderLookup.Provider> damageTypes = lookupProvider.thenApply(r -> constructRegistries(r, new TLReGenRegistrySetBuilder().add(Registries.NOISE_SETTINGS, null)));
	...
	protected static NoiseGeneratorSettings noiseSettings(boolean aquifersEnabled, BlockState defaultBlock, BlockState defaultFluid, boolean disabledMobGeneration, boolean legacyRandomSource, NoiseSettings noise, NoiseRouter noiseRouter, boolean oreVeinsEnabled, int seaLevel, List<Climate.ParameterPoint> spawnTarget, SurfaceRules.RuleSource surfaceRule) {
	...
	protected static NoiseSettings noise(int height, int minY, int sizeHorizontal, int sizeVertical) {
