...
public class TLReGenWorldgenTemplatePool extends TLReGenResourceGenerator {
	...
	public TLReGenWorldgenTemplatePool(DynamicRegister<StructureTemplatePool> dynReg, String modID, PackOutput packOutput) {
		super(modID, Types.STRUCTURE_TEMPLATE_POOL, packOutput);
		dynamicRegister = dynReg;
	}

	@Override
	public CompletableFuture<?> run(final CachedOutput cache) {
		return MasterResourceGenerator.lookupProvider.thenApply(r -> constructRegistries(r, new TLReGenRegistrySetBuilder().add(Registries.TEMPLATE_POOL, TLReGenWorldgenTemplatePool::bootstrap))).thenCompose((provider) -> {
