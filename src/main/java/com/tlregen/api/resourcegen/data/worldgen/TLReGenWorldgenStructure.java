...
public class TLReGenWorldgenStructure extends MasterResourceGenerator implements DataProvider {
	private final CompletableFuture<HolderLookup.Provider> completables = lookupProvider.thenApply(r -> constructRegistries(r, new TLReGenRegistrySetBuilder().add(Registries.STRUCTURE, TLReGenWorldgenStructure::bootstrap)));
	...
	public static void bootstrap(final BootstapContext<Structure> bootstrapContext) {
		TLReGenWorldgenStructure.setBootstrapContext(bootstrapContext);
	...
	protected static void setBootstrapContext(BootstapContext<Structure> bootstrapContextIn) {
