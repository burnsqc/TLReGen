...
public abstract class TLReGenTagsEntityTypes extends TLReGenMasterResourceGenerator implements DataProvider {
	...
	private final CompletableFuture<TagsProvider.TagLookup<EntityType<?>>> parentProvider = CompletableFuture.completedFuture(TagsProvider.TagLookup.empty());
	protected final ResourceKey<? extends Registry<EntityType<?>>> registryKey = Registries.ENTITY_TYPE;
	private final net.minecraftforge.common.data.ExistingFileHelper.IResourceType resourceType = new net.minecraftforge.common.data.ExistingFileHelper.ResourceType(net.minecraft.server.packs.PackType.SERVER_DATA, ".json", TagManager.getTagDir(Registries.ENTITY_TYPE));
	private final net.minecraftforge.common.data.ExistingFileHelper.IResourceType elementResourceType = new net.minecraftforge.common.data.ExistingFileHelper.ResourceType(net.minecraft.server.packs.PackType.SERVER_DATA, ".json", net.minecraftforge.common.ForgeHooks.prefixNamespace(Registries.ENTITY_TYPE.location()));
	@SuppressWarnings("deprecation")
	private final Function<EntityType<?>, ResourceKey<EntityType<?>>> keyExtractor = block -> block.builtInRegistryHolder().key();
	...
	public CompletableFuture<?> run(final CachedOutput cache) {
		...
		return lookupProvider.thenApply((provider) -> {
			...
			HolderLookup.RegistryLookup<EntityType<?>> registrylookup = combinedData.contents.lookup(registryKey).get();
			...
			return CompletableFuture.allOf(blockTags.entrySet().stream().map((p_255499_) -> {
				...
				if (!list1.isEmpty()) {
					...
					return DataProvider.saveStable(cache, json, packOutput.createPathProvider(PackOutput.Target.DATA_PACK, TagManager.getTagDir(Registries.ENTITY_TYPE)).json(resourcelocation));
				}
			}).toArray((p_253442_) -> {
	...
	public final String getName() {
		return "data." + modid + ".tags.entity_types";
	}
	...
	protected Path getPath(ResourceLocation id) {
		return packOutput.createPathProvider(PackOutput.Target.DATA_PACK, TagManager.getTagDir(Registries.ENTITY_TYPE)).json(id);
	}

	protected TLReGenTagsEntityTypes.IntrinsicTagAppender<EntityType<?>> tag(TagKey<EntityType<?>> p_255730_) {
		TagBuilder tagbuilder = this.getOrCreateRawBuilder(p_255730_);
		return new TLReGenTagsEntityTypes.IntrinsicTagAppender<EntityType<?>>(tagbuilder, this.keyExtractor, modid);
	}

	protected TagBuilder getOrCreateRawBuilder(TagKey<EntityType<?>> p_236452_) {
	...
	public CompletableFuture<TagsProvider.TagLookup<EntityType<?>>> contentsGetter() {
	...
	public static class IntrinsicTagAppender<T> extends TagsProvider.TagAppender<T> implements net.minecraftforge.common.extensions.IForgeIntrinsicHolderTagAppender<T> {
		...
		@Override
		public TLReGenTagsEntityTypes.IntrinsicTagAppender<T> addTag(TagKey<T> p_256311_) {
			super.addTag(p_256311_);
			return this;
		}

		public final TLReGenTagsEntityTypes.IntrinsicTagAppender<T> add(T p_256557_) {
			this.add(this.keyExtractor.apply(p_256557_));
			return this;
		}

		@SafeVarargs
		public final TLReGenTagsEntityTypes.IntrinsicTagAppender<T> add(T... p_255868_) {
