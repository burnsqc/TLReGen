...
public class TLReGenWorldgenPlacedFeature extends TLReGenResourceGenerator {
	public static BootstapContext<PlacedFeature> bootstrapContext;
	public static DynamicRegister<PlacedFeature> dynamicRegister;

	public TLReGenWorldgenPlacedFeature(DynamicRegister<PlacedFeature> dynReg, String modID, PackOutput packOutput) {
		super(modID, Types.PLACED_FEATURE, packOutput);
		dynamicRegister = dynReg;
	}

	@Override
	public CompletableFuture<?> run(final CachedOutput cache) {
		return MasterResourceGenerator.lookupProvider.thenApply(r -> constructRegistries(r, new TLReGenRegistrySetBuilder().add(Registries.PLACED_FEATURE, TLReGenWorldgenPlacedFeature::bootstrap))).thenCompose((provider) -> {
	...
	private static void bootstrap(final BootstapContext<PlacedFeature> bootstrapContextIn) {
