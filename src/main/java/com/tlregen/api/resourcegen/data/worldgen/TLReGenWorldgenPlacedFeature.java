...
public class TLReGenWorldgenPlacedFeature extends MasterResourceGenerator implements DataProvider {
	...
	public CompletableFuture<?> run(final CachedOutput cache) {
		return lookupProvider.thenApply(r -> constructRegistries(r, new TLReGenRegistrySetBuilder().add(Registries.PLACED_FEATURE, TLReGenWorldgenPlacedFeature::bootstrap))).thenCompose((provider) -> {
	...
	private <T> Optional<CompletableFuture<?>> dumpRegistryCap(CachedOutput cache, HolderLookup.Provider lookupProvider, DynamicOps<JsonElement> dynamicops, RegistryDataLoader.RegistryData<T> registryData) {
		return lookupProvider.lookup(registryData.key()).map((registryLookup) -> {
			return CompletableFuture.allOf(registryLookup.listElements().map((reference) -> {
				...
				return DataProvider.saveStable(cache, json, packOutput.createPathProvider(PackOutput.Target.DATA_PACK, "worldgen/placed_feature").json(reference.key().location()));
