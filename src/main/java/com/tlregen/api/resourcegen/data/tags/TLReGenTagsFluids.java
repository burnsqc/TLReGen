...
public abstract class TLReGenTagsFluids extends TLReGenMasterResourceGenerator implements DataProvider {
	...
	private final CompletableFuture<TagsProvider.TagLookup<Fluid>> parentProvider = CompletableFuture.completedFuture(TagsProvider.TagLookup.empty());
	protected final ResourceKey<? extends Registry<Fluid>> registryKey = Registries.FLUID;
	private final net.minecraftforge.common.data.ExistingFileHelper.IResourceType resourceType = new net.minecraftforge.common.data.ExistingFileHelper.ResourceType(net.minecraft.server.packs.PackType.SERVER_DATA, ".json", TagManager.getTagDir(Registries.FLUID));
	private final net.minecraftforge.common.data.ExistingFileHelper.IResourceType elementResourceType = new net.minecraftforge.common.data.ExistingFileHelper.ResourceType(net.minecraft.server.packs.PackType.SERVER_DATA, ".json", net.minecraftforge.common.ForgeHooks.prefixNamespace(Registries.FLUID.location()));
	@SuppressWarnings("deprecation")
	private final Function<Fluid, ResourceKey<Fluid>> keyExtractor = block -> block.builtInRegistryHolder().key();
	...
	public CompletableFuture<?> run(final CachedOutput cache) {
		...
		return lookupProvider.thenApply((provider) -> {
			...
			HolderLookup.RegistryLookup<Fluid> registrylookup = combinedData.contents.lookup(registryKey).get();
			...
			return CompletableFuture.allOf(blockTags.entrySet().stream().map((p_255499_) -> {
				...
				if (!list1.isEmpty()) {
					...
					return DataProvider.saveStable(cache, json, packOutput.createPathProvider(PackOutput.Target.DATA_PACK, TagManager.getTagDir(Registries.FLUID)).json(resourcelocation));
				}
			}).toArray((p_253442_) -> {
	...
	protected Path getPath(ResourceLocation id) {
		return packOutput.createPathProvider(PackOutput.Target.DATA_PACK, TagManager.getTagDir(Registries.FLUID)).json(id);
	}

	protected TLReGenTagsFluids.IntrinsicTagAppender<Fluid> tag(TagKey<Fluid> p_255730_) {
		TagBuilder tagbuilder = this.getOrCreateRawBuilder(p_255730_);
		return new TLReGenTagsFluids.IntrinsicTagAppender<Fluid>(tagbuilder, this.keyExtractor, modid);
	}

	protected TagBuilder getOrCreateRawBuilder(TagKey<Fluid> p_236452_) {
	...
	public CompletableFuture<TagsProvider.TagLookup<Fluid>> contentsGetter() {
	...
	public static class IntrinsicTagAppender<T> extends TagsProvider.TagAppender<T> implements net.minecraftforge.common.extensions.IForgeIntrinsicHolderTagAppender<T> {
		...
		@Override
		public TLReGenTagsFluids.IntrinsicTagAppender<T> addTag(TagKey<T> p_256311_) {
			super.addTag(p_256311_);
			return this;
		}

		public final TLReGenTagsFluids.IntrinsicTagAppender<T> add(T p_256557_) {
			this.add(this.keyExtractor.apply(p_256557_));
			return this;
		}

		@SafeVarargs
		public final TLReGenTagsFluids.IntrinsicTagAppender<T> add(T... p_255868_) {
